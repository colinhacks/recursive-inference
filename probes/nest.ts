...
interface NullInternals<T extends Schema> extends Internals<Out<T> | null> {}
interface NullSchema<T extends Schema = Schema> extends Schema { _zod: NullInternals<T> }
declare function nul<T extends Schema>(el: T): NullSchema<T>;
...
// nested wrapper chain, matching zod's ZodNullable<ZodOptional<ZodArray<...>>>
const A = obj({ title: str(), get kids() { return nul(opt(arr(A))); } });
